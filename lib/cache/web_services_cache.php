...
class MlrwsCache{
		...
		if( !$force_reload ){
			...
			if( !empty($cached_web_service_infos) ){
				if( !empty($last_update) ){
					if( $last_update === 'get' ){
						$cached_web_service_content = self::get_last_update_answer($cached_web_service_infos['timestamp']);
					}else{
	 ...
	 * @param string $web_service_slug To delete all caches, set $web_service_slug = 'mlrws-delete-all-caches'
	 */
	public static function delete_web_service_cache($web_service_slug_or_prefix){
		...
		if( !empty($web_service_slug_or_prefix) && file_exists($path) ){
			if( $handle = opendir($path) ){
				...
				$delete_all = $web_service_slug_or_prefix == 'mlrws-delete-all-caches';
				while( false !== ($entry = readdir($handle)) ){
					if ($entry != "." && $entry != "..") {
						if( $delete_all ){
	...
	private static function get_last_update_answer($cached_last_update){
		$last_update_answer = array('result' => (object)array('status'=>1,'message'=>''), 'last-update' => $cached_last_update );
		if( function_exists('apply_filters') ){ //We can pass here when Wordpress API is not loaded yet... 
			$last_update_answer = apply_filters('mlrws_last_update_answer',$last_update_answer,$cached_last_update);
		}
		return json_encode((object)$last_update_answer);
	}
}